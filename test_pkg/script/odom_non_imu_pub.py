...
while not rospy.is_shutdown():    
    ...
    if t[0] == "Quaternion":
        x_imu = float(t[1].split(", ")[0])
        y_imu = float(t[1].split(", ")[1])
        z_imu = float(t[1].split(", ")[2])
        w_imu = float(t[1].split(", ")[3])
    
    if t[0] == "Encoder":
        encoder = int(t[1].split(", ")[0].split('\r')[0])/1000
        
        vx = 0
        vy = 0
        vth = 0
        vz = (encoder - encoder_old)/dt
